...
export default function PostListModal({ visible, onClose }: PostListModalProps) {
            ...
            <View>
                {item.media.map((media: any, index: number) => {
                    ...
                    if (media.media_type === 'image') {
                        ...
                        return <Video key={index} source={{ uri: mediaUrl }} style={styles.media} useNativeControls resizeMode={ResizeMode.CONTAIN} />;
                    }
                    return null;
                })}
            </View>
    ...
    if (loading) {
        ...
        content = <Text style={styles.centered}>投稿はまだありません。</Text>;
    } else {
